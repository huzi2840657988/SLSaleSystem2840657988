			...
			$.ajax({
				...
				success:function(result){
					if(result != "" && "success" == result){
						msgtip.css("color","green");
						msgtip.html("修改密码成功 ！");
						alert("修改密码成功 ，下次登录记得使用新密码哦。^_^");
						window.location.href='/SLSaleSystem/main.html';
					}else if("failed" == result){
						msgtip.css("color","red");
						msgtip.html("修改密码失败！请重试。");
					}else if("oldpwdwrong" == result){
						msgtip.css("color","red");
						msgtip.html("原密码不正确！请重试。");
					}else if("nodata" == result){
						msgtip.css("color","red");
						msgtip.html("对不起，没有任何数据需要处理！请重试。");
					}
				}
				});
		}
...
//使用循环菜单列表（动态配置菜单）
	for(var i = 0;i<json.length;i++){
		...
		for(var j=0;j < json[i].subMenu.length;j++){
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\" href=\""+"/"+"SLSaleSystem"+json[i].subMenu[j].funcUrl +"\"><i class="+pic+"></i><span class=\"hidden-tablet\">"+json[i].subMenu[j].functionName + "</span></a></li>";
		}
		result = result +"</ul></li>";
	}
	$("#menus").append(result);
