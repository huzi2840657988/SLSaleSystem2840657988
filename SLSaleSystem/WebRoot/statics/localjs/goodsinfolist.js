...
$('.delgoodsinfo').click(function(e){
	...
	if(confirm("您确定要删除【"+d_goodsName+"】这个商品吗？")){
		//delete
		$.post("/SLSaleSystem/backend/delgoodsinfo.html",{'delId':d_id},function(result){
			if("success" == result){
				alert("删除成功！");
...
$("#a_goodsSN").blur(function(){
	...
	if(agsn != ""){
		$.post("/SLSaleSystem/backend/goodsSNisexit.html",{'goodsSN':agsn,'id':'-1'},function(result){
			if(result == "repeat"){
...
$("#m_goodsSN").blur(function(){
	...
	if(agsn != ""){
		$.post("/SLSaleSystem/backend/goodsSNisexit.html",{'goodsSN':agsn,'id':$("#m_id").val()},function(result){
			if(result == "repeat"){
...
$(".modifystate").click(function(){
	...
	$.ajax({
		url: '/SLSaleSystem/backend/modifystate.html',
		type: 'POST',
