...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="/SLSaleSystem/backend/goodspacklist.html">商品套餐管理</a></li>
	</ul>
</div>
				...
				<div class="box span12">
					...
					<div class="pagination pagination-centered">
					  ...
					  <c:choose>
					  	...
					  	<c:otherwise>
					  	<li><a href="/SLSaleSystem/backend/goodspacklist.html?currentpage=1&s_goodsPackName=${s_goodsPackName}&s_state=${s_state}&s_typeId=${s_typeId}" title="首页">首页</a></li>
					  	</c:otherwise>
					  </c:choose>
						<c:if test="${page.prevPages!=null}">
							<c:forEach items="${page.prevPages}" var="num">
								<li><a href="/SLSaleSystem/backend/goodspacklist.html?currentpage=${num}&s_goodsPackName=${s_goodsPackName}&s_state=${s_state}&s_typeId=${s_typeId}"
						...
						<c:if test="${page.nextPages!=null}">
							<c:forEach items="${page.nextPages}" var="num">
								<li><a href="/SLSaleSystem/backend/goodspacklist.html?currentpage=${num}&s_goodsPackName=${s_goodsPackName}&s_state=${s_state}&s_typeId=${s_typeId}" title="${num}">
								${num} </a></li>
							</c:forEach>
						</c:if>
						<c:if test="${page.pageCount !=null}">
							<c:choose>
						  	...
						  	<c:otherwise>
						  	<li><a href="/SLSaleSystem/backend/goodspacklist.html?currentpage=${page.pageCount}&s_goodsPackName=${s_goodsPackName}&s_state=${s_state}&s_typeId=${s_typeId}" title="尾页">尾页</a></li>
						  	</c:otherwise>
						    </c:choose>
					    </c:if>
						<c:if test="${page.pageCount == null}">
...
<script type="text/javascript" src="/SLSaleSystem/statics/localjs/goodspacklist.js"></script> 
